...
interface ChartData {  image: string;
  error?: string;}
export default function DurationVitalsCorrelation() {
  ...
  useEffect(() => {    const fetchChart = async () => {
      try {        const response = await fetch('http://localhost:8000/api/plot/duration-vitals-correlation');
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4 p-4">      <h2 className="text-xl font-semibold">Duration, Vitals & Calories Correlation Matrix</h2>
      <div className="w-full max-w-3xl">        <img 
          src={`data:image/png;base64,${chartData.image}`}          alt="Duration, vitals and calories correlation matrix"
