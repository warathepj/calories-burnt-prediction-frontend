...
interface ChartData {  image: string;
  error?: string;}
export default function CaloriesByBodyTemp() {
  ...
  useEffect(() => {    const fetchChart = async () => {
      try {        const response = await fetch('http://localhost:8000/api/plot/calories-by-body-temp');
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4 p-4">      <h2 className="text-xl font-semibold">Calories Burned vs Body Temperature</h2>
      <div className="w-full max-w-3xl">        <img 
          src={`data:image/png;base64,${chartData.image}`}          alt="Calories burned vs body temperature"
